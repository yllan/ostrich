  ...
  "Stats" should {
    ...
    "timings" in {
      ...
      "boundary timing sizes" in {
        ...
        val sum = 5.0 + scala.Int.MaxValue
        val avg = sum / 2.0
        val sumsq = 5.0 * 5.0 + scala.Int.MaxValue.toDouble * scala.Int.MaxValue.toDouble
        val partial = sumsq - sum * avg
        val test = Stats.getTiming("test")
        test.get(true) mustEqual
          new TimingStat(2, scala.Int.MaxValue, 5, Some(Histogram(5, scala.Int.MaxValue)), avg, partial)
      }
